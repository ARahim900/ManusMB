// Comprehensive STP Plant Database Service with Full User Data
...
// Monthly Performance Summary Data - Based on User's Complete Database
export const monthlyPerformanceData = [
  ...
  },
  {
    month: 'June 2025',
    monthKey: '2025-06',
    operatingDays: 11, // Partial month data
...
export const getDataByMonth = (monthKey) => {
  return monthlyPerformanceData.find(m => m.monthKey === monthKey);
};
...
export const getAnnualSummary = () => {
  ...
  const thisYearData = monthlyPerformanceData.filter(m => 
    m.monthKey.startsWith(currentYear.toString())
  );
  
  const lastYearData = monthlyPerformanceData.filter(m => 
    m.monthKey.startsWith((currentYear - 1).toString())
  );
  
  return {
    thisYear: {
      totalTreated: thisYearData.reduce((sum, m) => sum + m.totalTreatedWater, 0),
      totalTSE: thisYearData.reduce((sum, m) => sum + m.totalTSEWater, 0),
      totalBenefit: thisYearData.reduce((sum, m) => sum + m.totalFinancialBenefit, 0),
      months: thisYearData.length
    },
    lastYear: {
      totalTreated: lastYearData.reduce((sum, m) => sum + m.totalTreatedWater, 0),
      totalTSE: lastYearData.reduce((sum, m) => sum + m.totalTSEWater, 0),
      totalBenefit: lastYearData.reduce((sum, m) => sum + m.totalFinancialBenefit, 0),
      months: lastYearData.length
    }
  };
};
