...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, LineChart, Line, Label, ComposedChart } from 'recharts';
import { LayoutDashboard, TrendingUp, Combine, BarChart2, CalendarDays, Target, Sparkles, X, Droplets, Recycle, Gauge, Database, Activity, CheckCircle, Power } from 'lucide-react';
import { stpRawData } from '../../data/stpData';
import { parseSTPData, PLANT_DESIGN_CAPACITY } from '../../data/stpParser';
...
const STPModule: React.FC = () => {
  ...
  const initialStpData = useMemo(() => parseSTPData(stpRawData), []);
  ...
  const monthlyData = useMemo(() => {
    ...
    return Object.values(monthlyMap).map(month => ({
      ...
      efficiency: Math.round((month.treatedWater / month.totalInlet) * 1000) / 10,
      irrigationEff: Math.round((month.tseOutput / month.treatedWater) * 1000) / 10,
      capacityUtilization: Math.round((month.treatedWater / month.days / PLANT_DESIGN_CAPACITY) * 1000) / 10
    })).sort((a, b) => {
